...
$(document).ready(function() {
  ...
  $('#tag-input').on('input', function() {
      var query = $(this).val().trim(); // Trim whitespace from the input value
      if (query.length > 0) {
          $.ajax({
              ...
              success: function(data) {
                  console.log(data); // Print the raw data received from the server to the console
                  var tags = JSON.parse(data);
                  console.log(tags); // Print the parsed tags array to the console for further inspection
                  ...
                  $('#suggestions').show(); // Show suggestions dropdown
              }
          });
      } else {
          $('#suggestions').empty().hide(); // Hide suggestions dropdown if input is empty
      }
  });

  // Handle selecting a tag from the suggestions
  $(document).on('click', '#suggestions div', function() {
      var tag = $(this).text().trim(); // Get the clicked tag and trim whitespace
      if (!selectedTags.includes(tag)) {
          selectedTags.push(tag);
          addTag(tag);
          updateHiddenInput();
      }
      $('#tag-input').val(''); // Clear input field after inserting tag
      $('#suggestions').hide(); // Hide suggestions dropdown
  });

  // Function to add a tag element to the container
  function addTag(tag) {
